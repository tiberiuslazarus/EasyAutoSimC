...
if (typeof $WowheadPower == "undefined") {
    var $WowheadPower = new function() {
        ...
        var opt = {
            ...
            screenModeWidthLimit = 550,
            currentType, currentId, currentLocale, currentDomain, currentParams, currentA, cursorX, cursorY, mode = MODES["float"],
            eventAttached = false,
            ...
            TYPE_BUILDING = 20,
            TYPE_FOLLOWER = 21,
            TYPE_MISSIONABILITY = 22,
            TYPE_MISSION = 23,
            TYPE_SHIP = 25,
            TYPE_THREAT = 26,
            TYPE_RESOURCE = 27,
            TYPE_CHAMPION = 28,
            TYPE_ORDERADVANCEMENT = 30,
            TYPE_AFFIX = 40,
            TYPE_GUIDE = 100,
        ...
        function init() {
            if (isRemote) {
                ...
                script.src = (document.location.protocol != "https:" ? "http:" : document.location.protocol) + "//wow.zamimg.com/js/basic.js?8";
        ...
        this.init = function() {
            if (isRemote) {
                $WH.ae(head, $WH.ce("link", {
                    type: "text/css",
                    href: (document.location.protocol != "https:" ? "http:" : document.location.protocol) + "//wow.zamimg.com/css/basic.css?11",
                    rel: "stylesheet"
                }))
            }
            attachEvent();
        ...
        function scanElement(t, e) {
            ...
            try {
                if (t.dataset && t.dataset.hasOwnProperty("wowhead")) {
                    ...
                    if (t.getAttribute && t.getAttribute("data-wowhead")) {
            ...
            if ($WH.isTouch() && t.getAttribute("data-disable-wowhead-touch-tooltip") == "true") {
                return
            }
            var i0, i1, i2, m, params = {};
            currentParams = params;
            var p = function(m, k, v) {
                if (k == "buff" || k == "sock" || k == "map" || k == "notip" || k == "chal" || k == "twtbc" || k == "twwotlk" || k == "twcata" || k == "pvp" || k == "noimage") {
                    params[k] = true
                } else {
                    if (k == "rand" || k == "ench" || k == "tink" || k == "lvl" || k == "spec" || k == "c" || k == "diff" || k == "diffnew" || k == "upgd" || k == "pwr" || k == "q" || k == "level" || k == "dd" || k == "ddsize" || k == "rank" || k == "gem1lvl" || k == "gem2lvl" || k == "gem3lvl" || k == "artk") {
                        params[k] = parseInt(v)
                    } else {
                        if (k == "gems" || k == "pcs" || k == "forg" || k == "know" || k == "cri" || k == "bonus" || k == "abil" || k == "rewards" || k == "gem1bonus" || k == "gem2bonus" || k == "gem3bonus") {
                            params[k] = v.split(":")
                        } else {
                            if (k == "who" || k == "domain") {
                                params[k] = v
            ...
            if (opt.applyto & 1) {
                ...
                if (t.href.indexOf("http://") == 0 || t.href.indexOf("https://") == 0) {
                    i0 = 1;
                    m = t.href.match(/^https?:\/\/(.+?)?\.?(?:wowhead)\.com(?:\:\d+)?\/\??(item|quest|spell|zone|achievement|event|itemset|item-set|transmog-set|outfit|guide|statistic|currency|npc|object|pet-ability|petability|building|follower|champion|garrisonability|mission-ability|mission|ship|threat|resource|order-advancement|affix)=([0-9\.-]+)/);
                    if (!m) {
                        m = t.href.match(/^https?:\/\/(.+?)?\.?(?:wowhead)\.com(?:\:\d+)?\/(guide)s\/([^\?&]+)/)
                    }
                    showLogo = 0
                } else {
                    relativeUrl = true;
                    m = t.href.match(/()\/\??(item|quest|spell|zone|achievement|event|itemset|item-set|transmog-set|outfit|statistic|currency|npc|object|pet-ability|petability|building|follower|champion|garrisonability|mission-ability|mission|ship|threat|resource|order-advancement|affix|guide)=([0-9\.-]+)/);
                    if (!m) {
                        m = t.href.match(/()\/(guide)s\/([^\?&]+)/)
                    }
                    showLogo = 1
                }
            }
            if (m == null && rel && (opt.applyto & 2)) {
                i0 = 0;
                i1 = 1;
                i2 = 2;
                m = rel.match(/(item|quest|spell|zone|achievement|event|itemset|item-set|transmog-set|outfit|statistic|currency|npc|object|pet-ability|petability|building|follower|champion|garrisonability|mission-ability|mission|ship|threat|resource|order-advancement|affix|guide).?([0-9\.-]+)/);
                showLogo = 1;
                relativeUrl = true
            }
            t.href.replace(/([a-zA-Z0-9]+)=?([a-zA-Z0-9:-]*)/g, p);
            if (rel) {
                rel.replace(/([a-zA-Z0-9]+)=?([a-zA-Z0-9:-]*)/g, p)
            }
            ...
            if (m) {
                ...
                if (useScreenMode) {
                    ...
                    if ((t.parentNode.className.indexOf("icon") == 0 && t.parentNode.nodeName == "DIV") || t.getAttribute("data-whattach") == "icon") {
                        mode = MODES.attachToIcon
                    } else {
                        if ($WH.isTouch() || t.getAttribute("data-whattach") == "true") {
                            mode = MODES.attach
                        } else {
                ...
                var timeout = function(t) {
                    ...
                    if (wowhead_tooltips.iconizelinks && (type == TYPE_ITEM || type == TYPE_ACHIEVEMENT || type == TYPE_SPELL) && data.icon) {
                        ...
                        if (t.style.setProperty) {
                            t.style.setProperty("padding-left", "18px", "important")
                        } else {
                            t.style.paddingLeft = "18px"
                        }
                        t.style.verticalAlign = "center";
                        t.style.background = "url(" + (document.location.protocol != "https:" ? "http:" : document.location.protocol) + "//wow.zamimg.com/images/wow/icons/tiny/" + data.icon.toLocaleLowerCase() + ".gif) left center no-repeat"
                    }
        ...
        function onTouch(e) {
            ...
            var i = 0;
            var result;
            while (t != null && i < 5 && (result = scanElement(t, e)) == -2323) {
                t = t.parentNode;
                ++i
            }
            if (result === true) {
        ...
        function request(type, id, locale, stealth, params) {
            ...
            var p = "";
            for (var i in params) {
                if (i != "rand" && i != "ench" && i != "tink" && i != "gems" && i != "sock" && i != "diff" && i != "diffnew" && i != "upgd" && i != "chal" && i != "twtbc" && i != "twwotlk" && i != "twcata" && i != "pvp" && i != "lvl" && i != "spec" && i != "bonus" && i != "q" && i != "level" && i != "abil" && i != "dd" && i != "ddsize" && i != "rank" && i != "rewards" && i != "gem1lvl" && i != "gem2lvl" && i != "gem3lvl" && i != "gem1bonus" && i != "gem2bonus" && i != "gem3bonus" && i != "artk" && i != "transmog") {
                    continue
                }
