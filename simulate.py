...
def runSim(fightStyle, equippedGear, configProfile, metric, outputType="json", outputFileName=None, delete=True):
    ...
    subprocess.check_call(["simcraft/simc.exe", inputFile, output, "threads=1", "fight_style=%s" % fightStyle, "target_error=.2"], stdout=open(os.devnull, 'wb'), stderr=open(os.devnull, 'wb'))
